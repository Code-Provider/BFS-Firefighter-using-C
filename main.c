	...
	for (i=0 ; i<MAX;i++){
		for(j=0;j<MAX;j++){
			if(MAP[i][j].whatsin==fire){
				fire_location.x=i;
				fire_location.y=j;
				printf("on a trouver un danger de feu de degrée : %d",MAP[i][j].deg);
				sleep(2);
	...
	SDL_Rect position = {0,0};// pour parcourir notre surface
		for (i = 0; i < MAX; ++i)
		{
			for (j = 0; j < MAX; ++j)
			{
				...
				position.y = 35*i;
				if (i == 0 && j ==0) {
					SDL_BlitSurface(station,NULL,jardin,&position);
				}
